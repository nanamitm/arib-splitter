...
STDMETHODIMP CLAVFStreamInfo::CreateAudioMediaType(AVFormatContext *avctx, AVStream *avstream)
{
    ...
    // Some Japanese ARIB broadcast AAC streams carry a malformed Program Config
    // Element that makes the demuxer report a bogus channel count (e.g. 30) and a
    // zero sample rate in codecpar, even though every ADTS frame declares a valid
    // channel_configuration (typically stereo). If that bogus count reaches the
...
static FormatMapping subtitle_format_map[] = {
    ...
    { AV_CODEC_ID_ARIB_CAPTION,         &MEDIASUBTYPE_ASS },
};
// clang-format on
...
STDMETHODIMP CLAVFStreamInfo::CreateSubtitleMediaType(AVFormatContext *avctx, AVStream *avstream)
{
    ...
    if (avstream->codecpar->codec_id == AV_CODEC_ID_ARIB_CAPTION)
    {
        ...
        memcpy(mtype.pbFormat + sizeof(SUBTITLEINFO), assHeader.c_str(), headerLen);
    }
